...
show_help() {
    ...
    echo ""
    echo "Modes: segment | reconstruct (3d) | reconstruct_la (la_3d) | left_complete | full_pipeline (full) | labels"
    echo ""
    echo "Example (Full Pipeline):"
    echo "  ./run_docker.sh --gpu /host/path/to/subject001 full --output-dir /data --output-prefix subject001"
    echo ""
    echo "Example (Segmenting a single file):"
    echo "  ./run_docker.sh /host/path/to/subject001 segment --input /data/CINE_SAX.nii.gz --output-dir /data/outputs --output-prefix subject001 --view-type sax"
    echo ""
