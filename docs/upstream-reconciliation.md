...
| **Retrained cohort weights (three internal cohorts)** | B only, **not exported to A**. `2D_segmentation/model_weights/` and `.../results/`: `Dataset315` (2 folds), `Dataset316` (2), `Dataset317` (5), `Dataset400`, `Dataset401`, `Dataset402` (fold_all), `Dataset404` (2), `Dataset405` (5), `Dataset406` (5). Apr-May 2026, `nnUNetTrainer250epochs` or stock trainer. | Genuinely retrained 2D SAX/LAX models on three cohorts CardioForm has never seen. The strongest portable asset found. | **Medium-high.** Repackaging to `segment_*.zip` + `fold_all/checkpoint_final.pth` per `ModelManager` (plan §1.2); most are per-fold, not `fold_all`, so a fold choice or ensemble decision is needed first. Metrics **do** exist (found 2026-08-28): 29 `validation/summary.json` files. Cohort 1 foreground Dice 0.917-0.938 on n=11-12; cohort 3 0.812-0.900 on n=36-37; cohort 2 reports 0.982-0.991 but are `fold_all` runs, so not held out. **No UKBB baseline metrics exist**, so no cohort can be shown better than production. The first cohort's SAX and 2CH have only fold_0 validated and no `fold_all` model. |
